...
function getTime(date) {
  const fix2 = (val) => `00${val}`.slice(-2);
  return `${fix2(date.getHours())}:${fix2(date.getMinutes())}:${fix2(date.getSeconds())}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  const currentDate = new Date(year, currMonth, 1);
  let weekends = 0;
  while (currentDate.getMonth() === currMonth) {
    if (currentDate.getDay() === 0 || currentDate.getDay() === 6) weekends += 1;
...
function getWeekNumberByDate(date) {
  ...
  const januaryOne = new Date(currentDate.getFullYear(), 0, 1);
  const diffWeeks =
    Math.ceil(
      ((currentDate.getTime() - januaryOne.getTime()) / (24 * 3600 * 1000) +
        januaryOne.getDay()) /
        7
    ) + (januaryOne.getDay() === 0 ? 1 : 0);
  return diffWeeks;
}
...
function getNextFridayThe13th(date) {
  const currentDate = new Date(
    date.getFullYear(),
    date.getMonth() + (date.getDay() > 13 ? 1 : 0),
    13
  );

  while (currentDate.getDay() !== 5) {
    currentDate.setMonth(currentDate.getMonth() + 1, 13);
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const repeatSchedule = Array(countWorkDays)
    .fill(true)
    .concat(Array(countOffDays).fill(false));

  const dateCurrent = new Date(period.start.split('-').reverse().join('-'));
  const dateEnd = new Date(period.end.split('-').reverse().join('-'));

  let cursorSchedule = 0;
  while (dateCurrent <= dateEnd) {
    if (repeatSchedule[cursorSchedule])
      workSchedule.push(dateCurrent.toLocaleDateString('es-CL'));
    dateCurrent.setDate(dateCurrent.getDate() + 1);
    cursorSchedule += 1;
    if (cursorSchedule === repeatSchedule.length) cursorSchedule = 0;
  }
